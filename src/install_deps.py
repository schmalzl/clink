...
def install_build_dependencies():
    ...
    gui.CONSOLE_BUFFER.append("status_Updating build dependencies...")
    gui.CONSOLE_BUFFER.append("note_Depending on you connection speed, this process can take some time.")
    ...
    for i in deps_array_raw:
        gui.CONSOLE_BUFFER.append("status_Checking for an active " + i + " installation...")
        result = subprocess.run(["pip", "install", i], capture_output=True, text=True)
        # print to console
        gui.CONSOLE_BUFFER.append(result.stdout)
        gui.CONSOLE_BUFFER.append("ok_Executed pip command to install " + i)